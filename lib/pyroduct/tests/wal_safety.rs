...
//     let row = PyroRow::parse_wire(view.py_ref()).expect("parse ok");
...
//     for (i, rec) in recovered.iter().enumerate() {
//         match &records[i] {
//             WalRecord::Success { .. } => match rec {
//                 WalRecord::Success { row_index, .. } => assert_eq!(*row_index, i),
//                 _ => panic!("row {} expected Success, got Failure", i),
//             },
//             WalRecord::Failure { .. } => match rec {
//                 WalRecord::Failure { row_index, .. } => assert_eq!(*row_index, i),
//                 _ => panic!("row {} expected Failure, got Success", i),
//             },
//         }
//     }
// }
...
//     let row = PyroRow::parse_wire(view.py_ref()).expect("parse ok");
//     let restored: String = row.get("payload").unwrap().clone().into();
...
//         let row = PyroRow::parse_wire(view.py_ref()).expect("parse");
...
//     let r1 = PyroRow::parse_wire(v1.py_ref()).expect("r1");
//     let r2 = PyroRow::parse_wire(v2.py_ref()).expect("r2");
//     assert_eq!(r1.get("id"), Some(&PyroValue::from(1i32)));
//     assert_eq!(r2.get("id"), Some(&PyroValue::from(2i32)));

//     // Views should be independent
//     drop(v1);
//     let r2_still = PyroRow::parse_wire(v2.py_ref()).expect("r2 still valid");
