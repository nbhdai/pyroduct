...
fn test_execute_safe_large_payload() {
    ...
    assert_eq!(vec.status(), Ok(DataStatus::Valid));
    let typed = UserData::expose(vec).unwrap().into_owned::<UserData>();
...
fn test_execute_safe_panic_then_success() {
    ...
    let typed = UserData::expose(r2).unwrap().into_owned::<UserData>();
...
fn test_serialize_output_vec() {
    ...
    let restored = Vec::<u32>::expose(vec).unwrap().into_owned::<Vec<u32>>();
...
fn test_serialize_result_ok() {
    ...
    let typed = <Result<UserData, UserError>>::expose(vec)
        .unwrap()
        .into_owned::<Result<UserData, UserError>>()
        .unwrap();
...
fn test_full_roundtrip_user_data() {
    ...
    let recovered = UserData::expose(vec).unwrap().into_owned();
...
fn test_full_roundtrip_user_error() {
    ...
    let recovered = UserError::expose(vec).unwrap().into_owned();
    assert_eq!(original, recovered);
}
