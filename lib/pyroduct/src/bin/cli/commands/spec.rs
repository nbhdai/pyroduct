...
pub async fn spec(path: &Path, out: Option<&Path>) -> Result<()> {
    ...
    let (json, default_filename) = match &env.manifest {
        pyro_artifacts::cargo::ProjectManifest::Capability(_) => {
            let spec = env.capability_spec().await.context("Failed to generate capability spec")?;
            (serde_json::to_string_pretty(&spec).context("Failed to serialize capability spec")?, "interface.json")
        }
        pyro_artifacts::cargo::ProjectManifest::Module(_) => {
            let spec = env.module_spec().await.context("Failed to generate module spec")?;
            (serde_json::to_string_pretty(&spec).context("Failed to serialize module spec")?, "module.json")
        }
    };
