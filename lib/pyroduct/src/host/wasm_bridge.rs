...
pub struct HarnessState {
    ...
    // We hold these to make sure they stay loaded during execution
    pub capabilities: Vec<(String, Arc<Capability>)>,
}
...
impl HarnessState {
    ...
    pub fn capabilities(&self) -> impl Iterator<Item = &'_ str> {
        self.capabilities.iter().map(|(c, _)| c.as_str())
    }
}
