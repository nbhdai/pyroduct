use bridge_vec::{captured::{CapturedError, LibraryInfo}, library};
...
fn test_ffi_exposure_consistency() {
    ...
    let ffi_str = std::str::from_utf8(ffi_slice)
        .expect("FFI data must be valid UTF-8");
    
    println!("FFI Exposed JSON: {}", ffi_str);

    // 4. Deserialize the FFI string back into LibraryInfo
    let ffi_info: LibraryInfo = serde_json::from_str(ffi_str)
        .expect("FFI JSON must match the LibraryInfo schema");
