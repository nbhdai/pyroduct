...
use super::RowItem;
use super::{PyroRow, PyroRowOwned, PyroValue, PyroValueOwned, PrimitiveValueList};
use chrono::{DateTime, Utc};
// Needed for internal logic if used, or just public structs
use half::f16;

#[cfg(target_endian = "little")]
use super::{ArchivedPyroRow, ArchivedPyroValue, ArchivedPrimitiveValueList};
...
macro_rules! val_from_primitive {
    ($primitive_type:ty, $values_type:ident) => {
        ...
        // Option<Value>
        impl<'a> From<Option<$primitive_type>> for PyroValue<'a> {
...
impl From<Option<String>> for PyroValue<'static> {
    fn from(v: Option<String>) -> Self {
        match v {
            Some(v) => PyroValue::Str(Cow::Owned(v)),
            None => PyroValue::Null,
        }
    }
}
...
impl<'a> From<Option<&'a String>> for PyroValue<'a> {
...
impl<'a> From<Option<&'a str>> for PyroValue<'a> {
...
impl<'a, 'b> From<&'b ArchivedPyroValue<'a>> for PyroValue<'a> {
    fn from(archived: &'b ArchivedPyroValue<'a>) -> Self {
        match archived {
            ...
            ArchivedPyroValue::Timestamp(nanos) => {
                PyroValue::Timestamp(Time(nanos.0.to_native()))
            }
            ...
            ArchivedPyroValue::Group(archived_row) => {
                PyroValue::Group(PyroRow::from(archived_row))
            }
