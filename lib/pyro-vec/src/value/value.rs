...
pub struct PyroRow<'a>(pub(crate) Vec<RowItem<'a>>);
...
impl<'a> PyroRow<'a> {
    ...
    pub fn project(&self, fields: &[PyroField]) -> Result<PyroRow<'a>> {
...
impl<'a> PyroValue<'a> {
    ...
    pub fn into_owned(self) -> PyroValue<'static> {
        match self {
            ...
            PyroValue::List(l) => {
                PyroValue::List(l.into_iter().map(|v| v.into_owned()).collect())
            }
