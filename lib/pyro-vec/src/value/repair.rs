...
use super::{PyroRow, PyroValue, PrimitiveValueList, RowItem};
use crate::value::schema::{PyroField, PyroType, PrimitiveDataType};
...
impl<'a> PyroValue<'a> {
    ...
    pub fn repair_primitive_list<'b>(
        ...
        match fixed_len {
            ...
            _ => Err(ScalarRepairError::Cast(PyroType::PrimitiveList(
                inner_type,
            ))),
        }
    }
        ...
        macro_rules! match_cast {
            ($self_val:expr, $target_dt:expr) => {
                match ($self_val, $target_dt) {
                    ...
                    (_v, t) => Err(ScalarRepairError::Cast(PyroType::PrimitiveList(
                        t.clone(),
                    ))),
                }
            };
