...
use crate::{
    PyroVec, view::{PyroView, get_view}, wasm::host::{PyroState, WasmError}
};
...
impl<'a, T: Send + Sync + 'static> PyroCallIo<Caller<'a, PyroState<T>>> {
    ...
    pub fn from_caller(mut caller: Caller<'a, PyroState<T>>) -> Result<Self, WasmError> {
        ...
        Ok(Self { ctx: caller, memory })
    }
}

// Methods that require PyroState<T> as the store data.
impl<T: Send + Sync + 'static, S: AsContextMut<Data = PyroState<T>>> PyroCallIo<S> {
    ...
    pub async fn get_output(&mut self, ptr: i32) -> Result<PyroVec, WasmError> {
        ...
        let free_output = self
            ...
            .free_output();
        free_output.call_async(&mut self.ctx, ptr).await.map_err(|err| WasmError::OutputMemory(err.context("Unable to free an output")))?;
        Ok(vec)
    }
