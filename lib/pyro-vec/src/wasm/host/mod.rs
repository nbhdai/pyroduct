...
use crate::{
    PyroError, Bridgeable, CapturedError, format::Receiver, header::{PyroData, PyroHeader, DataStatus}, rkyv_8::RkyvReceiver, value::PyroRow, view::{PyroView}
};
...
pub struct PyroInstance<T: 'static> {
    store: Store<PyroState<T>>,
    instance: Instance,
    memory: Memory,
    receiver: RkyvReceiver<PyroRow<'static>>,
}

impl<T: Send + Sync + 'static> PyroInstance<T> {
    pub async fn new(
        engine: &PyroEngine,
        module: &PyroModule,
        linker: &PyroLinker<T>,
        ...
        let pyro_state = PyroState::new(state);
    ...
    pub async fn call(
        ...
        // We create a temporary IO scope
        ...
        // 2. Call the export
        // We must drop `io` before calling `call_async` because it borrows `store`.
        // Retrieve the typed function handle.
        ...
        // This reads the result and calls `free_output` in wasm automatically.
        let mut io = PyroCallIo::new(&mut self.store, self.memory);
        let output_vec = io
        ...
        match result_view.status() {
            ...
            _ => Err(PyroError::UnknownStatus(result_view.status_u8()).into()),
        }
    }
