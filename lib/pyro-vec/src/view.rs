use crate::{
    PyroVec,
    error::PyroErrorView,
    header::{PyroData, PyroHeader, PyroParser, MAGIC_VAL, MutPyroData},
};
use std::{
    fmt,
    ops::{Deref, DerefMut},
...
impl<'a> PyroMutView<'a> {
    ...
    pub fn copy_from_view(&mut self, view: &PyroView<'_>) -> Result<(), PyroErrorView> {
...
pub fn get_view<'a>(
    wasm_memory: &'a [u8],
    ptr_offset: usize,
) -> Result<PyroView<'a>, PyroErrorView> {
    let offset = ptr_offset as usize;

    // 1. Check Alignment
    // PyroVec mandates 16-byte alignment.
    if offset % PyroParser::ALIGN != 0 {
        return Err(PyroErrorView::MisalignedPointer);
    }

    // 2. Check Header Bounds
    // We need at least HEADER_SIZE bytes to read the header
    if offset
        .checked_add(PyroParser::HEADER_SIZE)
        .ok_or(PyroErrorView::OutOfBounds {
            required: PyroParser::HEADER_SIZE,
            available: 0,
        })?
        > wasm_memory.len()
    {
        return Err(PyroErrorView::OutOfBounds {
            required: offset + PyroParser::HEADER_SIZE,
    ...
    let magic_bytes: [u8; 4] = wasm_memory
        [offset + PyroParser::OFFSET_MAGIC..offset + PyroParser::OFFSET_MAGIC + 4]
        .try_into()
        .unwrap();
    let magic = u32::from_le_bytes(magic_bytes);

    if magic != MAGIC_VAL {
        return Err(PyroErrorView::InvalidHeader);
    }
...
pub fn get_view_mut<'a>(
    ...
    ptr_offset: usize,
) -> Result<PyroMutView<'a>, PyroErrorView> {
    let offset = ptr_offset as usize;
