...
/// Errors that can occur when creating or accessing a PyroView.
#[derive(Debug, Error)]
pub enum PyroErrorView {
    ...
    /// The buffer contained a PyroError (status code != 0).
    /// This wraps the standard PyroError which may allocate.
    #[error("pyro error: {0}")]
    PyroError(#[from] PyroError),
}
...
impl PyroError {
    ...
    pub fn library(&self) -> Option<&LibraryInfo<'static>> {
        match self {
            ...
            PyroError::UnknownStatus(_) => library(),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Pyro { origin, kind } => f
                ...
                .field("kind", kind)
                .finish(),
            Self::UnknownStatus(arg0) => f.debug_tuple("UnknownStatus").field(arg0).finish(),
        }
    }
}
...
impl From<PyroErrorView> for PyroError {
    fn from(err: PyroErrorView) -> Self {
        match err {
            // Unwrap the existing PyroError if present
            PyroErrorView::PyroError(e) => e,

            // Map invalid header directly to the ErrorKind
            PyroErrorView::InvalidHeader => PyroError::local(ErrorKind::InvalidHeader),

            // Map structural/memory errors to local Validation errors with context
            PyroErrorView::MisalignedPointer => {
                PyroError::validation(CapturedError::new("misaligned pointer"))
            }
            PyroErrorView::OutOfBounds {
                required,
                available,
            } => PyroError::validation(CapturedError::new(format!(
                "out of bounds: required {required} bytes, available {available}"
            ))),
        }
    }
}
