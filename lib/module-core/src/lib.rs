...
pub fn generate_module(content: &str) -> anyhow::Result<String> {
    let file = syn::parse_file(content)
        .map_err(|e| anyhow::anyhow!("Failed to parse module source file: {}", e))?;
...
fn extract_module_attr(attrs: &[syn::Attribute]) -> anyhow::Result<TokenStream> {
    for attr in attrs {
    ...
    Err(anyhow::anyhow!("No module attribute found"))
}
