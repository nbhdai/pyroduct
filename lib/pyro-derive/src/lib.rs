...
pub fn bridgeable(args: TokenStream, input: TokenStream) -> TokenStream {
    let args = parse_macro_input!(args as pyro_core::format::bridgeable::BridgeableArgs);
    let mut item = parse_macro_input!(input as ItemStruct);

    match pyro_core::format::bridgeable::bridgeable(&args, &mut item, &parse_quote!(::pyroduct)) {
        Ok(v) => v.into(),
...
pub fn magma(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    match pyro_core::format::magma(args, &mut item, &parse_quote!(::pyroduct)) {
        Ok(v) => v.into(),
        Err(error) => error.to_compile_error().into(),
    }
}
...
pub fn library(input: TokenStream) -> TokenStream {
    let args = parse_macro_input!(input as pyro_core::format::library::LibraryArgs);

    pyro_core::format::library::create_ident(syn::parse_quote!(::pyroduct), &args.meta, args.no_ffi)
        .into()
}
...
pub fn document(input: TokenStream) -> TokenStream {
    ...
    match pyro_core::format::documentation::generate_documented_impl(
        &item,
...
pub fn derive_deep_ref(input: TokenStream) -> TokenStream {
    let item = parse_macro_input!(input as ItemStruct);
    match pyro_core::format::deep_ref::deep_ref(&item, &parse_quote!(::pyroduct), &vec![]) {
        Ok(v) => v.into(),
        Err(error) => error.to_compile_error().into(),
    }
}
...
pub fn derive_deep_ref_archived(input: TokenStream) -> TokenStream {
    let item = parse_macro_input!(input as ItemStruct);
    match pyro_core::format::deep_ref::deep_ref_rkyv(&item, &parse_quote!(::pyroduct)) {
        Ok(v) => v.into(),
        Err(error) => error.to_compile_error().into(),
    }
}
...
pub fn derive_to_row(input: TokenStream) -> TokenStream {
    let item = parse_macro_input!(input as ItemStruct);
    match pyro_core::format::to_row::to_row(&item, &parse_quote!(::pyroduct)) {
        Ok(v) => v.into(),
        Err(error) => error.to_compile_error().into(),
    }
}
...
pub fn capability(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    match pyro_core::ffi::capability::CapabilityImpl::new(item, false, &name, &version) {
...
pub fn config(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    match pyro_core::ffi::config::CapConfig::new(item, DocRec::NoReq) {
...
pub fn module(attr: TokenStream, item: TokenStream) -> TokenStream {
    let attrs: pyro_core::module::ModuleAttrs = match parse2(attr.into()) {
        Ok(attrs) => attrs,
    ...
    pyro_core::module::expand(attrs, input_fn)
